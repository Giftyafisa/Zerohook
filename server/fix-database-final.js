...
console.log('🚀 Starting complete database schema fixes...');
...
async function fixDatabaseSchema() {
  ...
  try {
    ...
    // 4. Check if services table has any existing data and handle provider_id
    ...
    try {
      ...
      // Check if there's a user_id column or similar
      const hasUserId = tableInfo.rows.some(row => 
        row.column_name === 'user_id' || 
        row.column_name === 'provider_id' || 
        row.column_name === 'owner_id'
      );
      
      if (hasUserId) {
        console.log('✅ Found user identification column, updating provider_id...');
        // Try to find the correct column name
        const userIdColumn = tableInfo.rows.find(row => 
          row.column_name === 'user_id' || 
          row.column_name === 'owner_id'
        );
        ...
        if (userIdColumn) {
          await client.query(`
            UPDATE services 
            SET provider_id = ${userIdColumn.column_name} 
            WHERE provider_id IS NULL AND ${userIdColumn.column_name} IS NOT NULL;
          `);
          console.log(`✅ Updated provider_id from ${userIdColumn.column_name}`);
        }
      } else {
        console.log('⚠️ No user identification column found, skipping provider_id update');
      }
